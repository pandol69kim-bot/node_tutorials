...
const Sidebar = () => {
  ...
  return (
    <div className='dark:bg-gray-950 bg-white py-5 px-4 dark:shadow-[0_0.125rem_0.25rem_rgba(255,255,255,0.3)] shadow-[0_0.125rem_0.25rem_rgba(165,163,174,0.3)] w-[20%] flex flex-col rounded-sm z-[1000] fixed h-full'>
      ...
      <div className='sidebar-top mb-[30px] flex items-center justify-between'>
        <div className='sidebar-brand flex items-center justify-center gap-x-[12px]'>
          ...
          <span className='brand-text text-lg font-semibold dark:text-white text-gray-500'>Marshall</span>
        </div>
        <button className='sidebar-close-btn text-black p-[0.125rem] cursor-pointer hover:bg-gray-300'>
          <MdOutlineClose />
        </button>
      </div>
      <div className='sidebar-body flex items-center'>
        <BrowserRouter>
          <Routes>
            {
          ...
          <div className='sidbar-menu w-full'>
                 ...
                 MENU_LISTS.map((menu, index) => (
                  <li key={index} className='menu-item'>
                    <Link 
                      ...
                      className={`h-[44px] flex items-center gap-x-[14ps] py-0.5, px-5 font-medium w-full ${
                        index === currentTab ? 
